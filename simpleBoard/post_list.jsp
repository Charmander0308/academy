...
<body>
	...
	<h1>게시글 목록</h1>
		...
		try
		{
			...
			String insertQuery = "SELECT * FROM practice_board.post order by num desc";
		...
		while(rs.next())
			{%>
				<tr>
					...
					<td><a href="post_read.jsp?num=<%=rs.getInt("num")%>"><%=rs.getString("title") %></a></td>
					...
					<td>
						...
						<button type="button" value="수정" onClick="location.href='post_modify.jsp?num=<%=rs.getString("num")%>'">수정</button>
						<!-- 삭제 버튼을 누르면 post_delete_send.jsp로 연결되며 num 칼럼값을 parameter로 넘김 -->
						<button type="button" value="삭제" onClick="location.href='post_delete_send.jsp?num=<%=rs.getString("num")%>'">삭제</button>
					</td>
				</tr>
				<%
				}%>
